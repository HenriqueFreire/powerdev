...
static inline void ev_create_notify(session_t *ps, xcb_create_notify_event_t *ev) {
	...
	// Objetivo: Identificar nossa janela secreta pela WM_CLASS.
	// Passo 1: Obter a propriedade WM_CLASS da nova janela.
	xcb_get_property_cookie_t cookie = xcb_get_property(
		ps->c.c, 0, ev->window, XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 0, 128
	);
	xcb_get_property_reply_t *reply = xcb_get_property_reply(ps->c.c, cookie, NULL);

	if (reply) {
		const char *wm_class = xcb_get_property_value(reply);
		if (wm_class && strcmp(wm_class, "powerdev_secret") == 0) {
			log_info("!!! POWER-DEV: Janela secreta detectada! ID: %#010x, WM_CLASS: %s", ev->window, wm_class);
		}
		free(reply);
	}
