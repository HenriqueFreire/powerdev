...
bool win_update_class(struct x_connection *c, struct atom *atoms, struct win *w) {
	...
	// --- INÍCIO DA MODIFICAÇÃO POWER-DEV ---
	// Verifica se a classe da janela é o nosso sinal secreto.
	if (w->class_instance && strcmp(w->class_instance, "powerdev_secret") == 0) {
		w->is_secret = true;
		log_info("Janela secreta encontrada: %#010x", win_id(w));
	} else {
		w->is_secret = false;
	}
	// --- FIM DA MODIFICAÇÃO POWER-DEV ---
